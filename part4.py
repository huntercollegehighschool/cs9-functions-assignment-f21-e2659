...
def possibletriangle(side1, side2, side3): #do not change this line
  if abs(side1 + side2) > side3 and abs(side1 - side2) < side3:
    return True
  elif abs(side1 + side3) > side2 and abs(side1 - side3) < side2:
    return True
  elif abs(side2 + side3) > side1 and abs(side2 - side3) < side1:
    return True
  else:
    return False
